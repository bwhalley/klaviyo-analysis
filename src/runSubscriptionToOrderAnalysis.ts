/**
 * Script to analyze subscription to first order time using Klaviyo MCP data
 * 
 * This processes the actual events fetched from Klaviyo and calculates:
 * - Median and mean time from subscription to first order
 * - Cohort analysis
 ...
 */
function processSubscriptionEvents(events: any[]): Map<string, Date> {
 ...
 */
function processOrderEvents(events: any[]): Map<string, Date> {
 ...
 */
function matchSubscriptionsToOrders(
 ...
 */
function calculateStatistics(profiles: ProfileData[]): Statistics {
  ...
  const p90 = daysToOrder[Math.floor(daysToOrder.length * 0.90)] || 0
  ...
  return {
    ...
    conversionRate: profiles.length > 0 ? (withOrders.length / profiles.length) * 100 : 0,
 ...
 */
function generateCohortData(profiles: ProfileData[]): CohortDataPoint[] {
  ...
  for (const profile of profiles) {
    ...
    const cohortLabel = `Week of ${date.toISOString().split('T')[0]}`
  ...
  for (const [cohortKey, cohortProfiles] of cohorts.entries()) {
    const withOrders = cohortProfiles.filter(p => p.daysToFirstOrder !== null && p.daysToFirstOrder! >= 0)
    const daysToOrder = withOrders.map(p => p.daysToFirstOrder!).sort((a, b) => a - b)
    
    const avgDays = daysToOrder.length > 0
      ? daysToOrder.reduce((sum, d) => sum + d, 0) / daysToOrder.length
      : 0
    
    const medianDays = daysToOrder.length > 0
      ? daysToOrder[Math.floor(daysToOrder.length / 2)]
      : 0
    
    const firstProfile = cohortProfiles[0]
    cohortData.push({
      signupDate: firstProfile.firstSubscriptionDate.toISOString().split('T')[0],
      ...
      conversionRate: cohortProfiles.length > 0 ? (withOrders.length / cohortProfiles.length) * 100 : 0,
...
export async function analyzeSubscriptionToOrder(
  ...
  const subscriptions = processSubscriptionEvents(subscriptionEvents)
  ...
  const profiles = matchSubscriptionsToOrders(subscriptions, orders)
  
  console.log('Calculating statistics...')
  const statistics = calculateStatistics(profiles)
  
  console.log('Generating cohort data...')
  const cohortData = generateCohortData(profiles)
...
// Example usage:
// const subscriptionEvents = [...]; // From mcp_klaviyo_klaviyo_get_events
// const orderEvents = [...]; // From mcp_klaviyo_klaviyo_get_events
